...
> [!IMPORTANT]
> **Rayure 自 2026 年 9 月起长期暂停开发。** 为什么做、做到了哪、什么时候继续，见下方《暂停通知》。算力要求更低的后续尝试已移步 [ApolloEddy/Pliette](https://github.com/ApolloEddy/Pliette)。

## ⏸️ 暂停通知

### 这个项目想做成什么

让喜欢的角色真正「住」进桌面：一个有空间感的桌面场景，角色看得见你、听得到你说话，并根据当下的情境自然地说话和行动。动作可以随时中途改变；你移动头部时，还能看见场景里更多的细节。它不是一段循环播放的动画，而是一个与你共处一室的存在。

角色可以替换，场景可以替换，AI 后端也可以替换。

### 为什么要做这个

桌面壁纸向来是「你看着它」，我想试的是反过来——让它看着你。语音识别、视觉感知、动作生成这些能力，如今已经便宜到一个人就能拼出原型；一个能感知你、回应你的桌面角色伴侣，也许正站在「做得到」的边缘。Rayure 就是伸手去推这条边界的一次尝试。

### 试过了哪些路

围绕「ARDY 等人体动作 / 表情 / 微动作生成模型 + LLM 大脑」这条主线，我探索了多种实时交互通道：

- EMAGE / ARDY 动作序列 → Live2D
- EMAGE / ARDY 动作序列 → 常规 3D 模型
- EMAGE / ARDY 动作序列 → MediaPipe + MiKaPo → MMD 动作
- 实时 Text-to-Video 响应式桌面渲染

过程中也参考、借用了 AIRI 与 Wallpaper Engine 的环境和依赖。部分链路确实跑通了（语音 → 行为计划 → 动作与口型，Live2D 主线曾在浏览器里端到端演示），但受限于我在 3D 项目上的能力短板、RTX 4060 的算力上限等原因，没有一条路达到我期望的效果——离「自然、自由、实时」始终差着一截。

### 换一条更轻的路

对算力要求更低的尝试并没有停：我已在另一个仓库 **[ApolloEddy/Pliette](https://github.com/ApolloEddy/Pliette)**（Q 版 3D 交互小人 · AI 伴侣）开发 Q 版模型的桌面实时响应项目。如果你对「桌面角色伴侣」这件事本身感兴趣，欢迎移步那边——这条路轻得多，也走得动。

### 什么时候会继续

等到 RTX 4060 这个量级的消费级显卡，能够以一倍以上于实时的速度生成视频；或者出现更合适的实时动作生成模型——到那时，这个项目还能捡起来继续推进。

### 最后一点期望

这个仓库现在十分混乱，实现思路也很常规，大概没有什么借鉴意义。比起它自己被做完，我更期待之后有人真的把它做出来——让一个 3D 模型伴侣，真正住进每个人的电脑里。
